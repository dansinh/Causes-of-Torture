#############################################################
# Estimating Causal Variable Importance Using XGB-based Substitution
#############################################################
...
cvim_xgb <- data.frame(cbind(mean = mean_boot[1:23],
...
cvim_xgb$covariate <- factor(cvim_xgb$covariate,
                             levels = cvim_xgb$covariate[order(cvim_xgb$mean)])
fplot <- ggplot(data = cvim_xgb,
                aes(x = covariate, y = mean, ymin = lower, ymax = upper)) +
  geom_pointrange(size = 1.2) + geom_hline(yintercept = 0, lty = 2, size = 1) +
  coord_flip() + ggtitle("Causal effects of covariates on human rights protection") +
  theme_fivethirtyeight() + theme(axis.text = element_text(size = 20))
fplot

# Create LaTex table
colnames(cvim_xgb) <- c("Covariate", "Mean Causal Effect", "Lower", "Upper")
xtable(cvim_xgb, digits = c(rep(3, 5)))

# Save results
save.image("CVIM_XGB.RData")
