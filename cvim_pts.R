...
library(xtable) # Create LaTeX table of results
...
data <- read.csv("midata1.csv") %>%
  dplyr::select(c(year, ccode,
                  ...
                  britcol, common, amnesty, amnesty_lag,
...
cvim3_xgb <- data.frame(cbind(mean = mean_boot[1:23],
...
cvim3_xgb$covariate <- factor(cvim3_xgb$covariate,
                             levels = cvim3_xgb$covariate[order(cvim3_xgb$mean)])
fplot <- ggplot(data = cvim3_xgb,
                aes(x = covariate, y = mean, ymin = lower, ymax = upper)) +
  geom_pointrange(size = 1.2) + geom_hline(yintercept = 0, lty = 2, size = 1) +
  coord_flip() + ggtitle("Causal effects of covariates on political terror score") +
  theme_fivethirtyeight() + theme(axis.text = element_text(size = 20))
fplot

# Create LaTex table
colnames(cvim3_xgb) <- c("Covariate", "Mean Causal Effect", "Lower", "Upper")
xtable(cvim3_xgb, digits = c(rep(3, 5)))

# Save results
save.image("CVIM3_XGB.RData")
